...
{
  Serial.println("In ensureFSMounted");
  if (SPIFFS.begin())
  {
    ...
    if (SPIFFS.exists("/config.json"))
    {
      ...
      if (configFile)
      {
        ...
        if (json.success())
        {
          ...
          mqtt_secure = atoi(json["mqtt_secure"]);
        }
        else
        {
...
void callback(char *topic, byte *payload, unsigned int length)
{
  ...
  if (eventHappened)
  {
    ...
    if (json["_type"] == "transition")
    {
      ...
      if (region > 6) {
        region = 8; // lost, only allow 6 locations, 2 reserved.Cannot enter travelling thats a bodge.
    ...
    if (json["_type"] == "location")
    {
      ...
      if (region > 6)
        region = 8; // lost, only allow 6 locajson.printTo(Serial);tions, 2 reserved. Cannot enter travelling thats a bodge.
      if (region == 0)
        region = 8; // lost
      sendMoveToMotors(whichHand, ((region * stepsPerFace) - stepsPerFace) + (whichHand * perPersonStep) + stepFiller);
...
void setup()
{
  ...
  else
  {
    ...
    if (mqtt_secure)
    {
      Serial.println("Secure mqtt client - doubt this will work yet");
